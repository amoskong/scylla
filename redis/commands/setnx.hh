...
class setnx : public abstract_command {
    bytes _key;
    bytes _data;
    long _ttl = 0;
public:
    static shared_ptr<abstract_command> prepare(service::storage_proxy& proxy, request&& req);
    setnx(bytes&& name, bytes&& key, bytes&& data, long ttl)
        : abstract_command(std::move(name))
        , _key(std::move(key))
        , _data(std::move(data))
        , _ttl(ttl) {
    }
    setnx(bytes&& name, bytes&& key, bytes&& data)
        : setnx(std::move(name), std::move(key), std::move(data), 0) {
    }
