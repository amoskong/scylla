...
shared_ptr<abstract_command> setnx::prepare(service::storage_proxy& proxy, request&& req) {
    ...
    return seastar::make_shared<setnx> (std::move(req._command), std::move(req._args[0]), std::move(req._args[1]), 0);
}

future<redis_message> setnx::execute(service::storage_proxy& proxy, redis::redis_options& options, service_permit permit) {
